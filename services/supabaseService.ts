// @ts-nocheck
import { AuthUser, SendOTPOptions, SignUpResult, GoogleSignInResult } from '../template/auth/types';
import { safeSupabaseOperation, getSharedSupabaseClient } from '../template/core/client';
...
export class AuthService {
  ...
  async getCurrentUser(): Promise<AuthUser | null> {
    try {
      ...
      return await this.enrichUserWithProfile(authUser);

    } catch (error) {
      console.warn('[AuthService] getCurrentUser system error:', error);
      return null;
    }
  }

  async enrichUserWithProfile(authUser: AuthUser): Promise<AuthUser> {
    if (!authUser?.id) return authUser;

    try {
      // Use direct supabase query to avoid service cross-dependency issues
      ...
      if (error) {
        // If profile doesn't exist yet (new user), return base auth user
        return authUser;
      }
      ...
      if (profile) {
        return {
          ...
          chatPin: profile.chat_pin
        };
      }
    } catch (e) {
      console.warn('[AuthService] Profile enrichment error:', e);
    }
    return authUser;
  }
  ...
  // Method already defined at line 119 - removing duplicate broken one
  ...
  onAuthStateChange(callback: (user: AuthUser | null) => void) {
    try {
      ...
      const { data: { subscription } } = this.supabase.auth.onAuthStateChange(
        async (event, session) => {
          console.log(`[AuthService] Auth Event: ${event}, Session: ${session ? 'Active' : 'Missing'}`);
          ...
          if (session?.user) {
            const baseAuthUser = this.mapSessionToAuthUser(session.user);

            // 1. Only callback if ID changed (prevents loops)
            if (baseAuthUser.id !== lastUserId) {
              lastUserId = baseAuthUser.id;
              callback(baseAuthUser);
            }

            // 2. Fetch full profile
            this.enrichUserWithProfile(baseAuthUser).then(fullUser => {
              const fingerprint = JSON.stringify({
                un: fullUser.username,
                fn: fullUser.full_name,
                av: fullUser.avatar,
                age: fullUser.age
              });
              ...
              console.warn('[AuthService] Deferred profile load failed:', err);
            });
      ...
      console.error('[AuthService] Failed to set up auth state listener:', error);
      throw error;
    }
  }
}
